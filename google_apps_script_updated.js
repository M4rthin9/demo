...
logEvent(username, 'create_role', roleName, { permissions: permissionsInput });
 ...
 if (action === 'getAll') {
  ...
  // ===== VISITORS MANAGEMENT =====
  // Get all visitors (Superadmin only)
  if (action === 'getVisitors') {
    if (!hasPermission(username, 'manage_users')) {
      return jsonResp({ status: 'error', message: 'เฉพาะผู้ดูแลระบบ (Superadmin) เท่านั้นที่สามารถดูข้อมูลผู้เยี่ยมได้' });
    }
    const visitors = getAllVisitors();
    return jsonResp({ status: 'ok', visitors: visitors });
  }

  // Create visitor (Superadmin only)
  if (action === 'createVisitor') {
    if (!hasPermission(username, 'manage_users')) {
      return jsonResp({ status: 'error', message: 'เฉพาะผู้ดูแลระบบ (Superadmin) เท่านั้นที่สามารถสร้างผู้เยี่ยมได้' });
    }
    ...
    if (!name || !idCard) {
      return jsonResp({ status: 'error', message: 'กรุณากรอกชื่อและบัตรประชาชน' });
    }

    try {
      const visitorsSheet = getVisitorsSheet();
      const visitorsData = visitorsSheet.getDataRange().getValues();
      const visitorsHeaders = visitorsData[0];
      visitorsSheet.appendRow([name, idCard, phone || '', email || '', relation || '', new Date().toISOString()]);
      logEvent(username, 'create_visitor', '', { name, idCard });
      return jsonResp({ status: 'ok', message: 'สร้างผู้เยี่ยมสำเร็จ' });
    } catch(e) {
      return jsonResp({ status: 'error', message: 'สร้างผู้เยี่ยมไม่สำเร็จ: ' + e.toString() });
    }
  }

  // Delete visitor (Superadmin only)
  if (action === 'deleteVisitor') {
    if (!hasPermission(username, 'manage_users')) {
      return jsonResp({ status: 'error', message: 'เฉพาะผู้ดูแลระบบ (Superadmin) เท่านั้นที่สามารถลบผู้เยี่ยมได้' });
    }
    const id = body.id;
    if (!id) {
      return jsonResp({ status: 'error', message: 'ไม่พบรหัสผู้เยี่ยม' });
    }

    try {
      const visitorsSheet = getVisitorsSheet();
      const data = visitorsSheet.getDataRange().getValues();
      const idIdx = data[0].indexOf('idCard');
      if (idIdx === -1) {
        return jsonResp({ status: 'error', message: 'ไม่พบคอลัมน์บัตรประชาชน' });
      }

      for (let i = 1; i < data.length; i++) {
        if (String(data[i][idIdx]) === String(id)) {
          visitorsSheet.deleteRow(i + 1);
          logEvent(username, 'delete_visitor', '', { idCard: id });
          return jsonResp({ status: 'ok', message: 'ลบผู้เยี่ยมสำเร็จ' });
        }
      }
      return jsonResp({ status: 'error', message: 'ไม่พบผู้เยี่ยมที่ต้องการลบ' });
    } catch(e) {
      return jsonResp({ status: 'error', message: 'ลบผู้เยี่ยมไม่สำเร็จ: ' + e.toString() });
    }
  }

  // Update visitor (Superadmin only)
  if (action === 'updateVisitor') {
    if (!hasPermission(username, 'manage_users')) {
      return jsonResp({ status: 'error', message: 'เฉพาะผู้ดูแลระบบ (Superadmin) เท่านั้นที่สามารถแก้ไขผู้เยี่ยมได้' });
    }
    const idCard = body.idCard?.trim();
    const oldIdCard = body.oldIdCard?.trim();
    if (!idCard || !oldIdCard) {
      return jsonResp({ status: 'error', message: 'ไม่พบรหัสผู้เยี่ยม' });
    }
    ...
    try {
      ...
      const data = visitorsSheet.getDataRange().getValues();
      const headers = data[0];
      const idIdx = headers.indexOf('idCard');
      ...
      for (let i = 1; i < data.length; i++) {
        if (String(data[i][idIdx]) === String(oldIdCard)) {
          const row = i + 1;
          if (nameIdx > -1) visitorsSheet.getRange(row, nameIdx + 1).setValue(body.name || '');
          if (idIdx > -1) visitorsSheet.getRange(row, idIdx + 1).setValue(idCard);
          if (phoneIdx > -1) visitorsSheet.getRange(row, phoneIdx + 1).setValue(body.phone || '');
          if (emailIdx > -1) visitorsSheet.getRange(row, emailIdx + 1).setValue(body.email || '');
          if (relationIdx > -1) visitorsSheet.getRange(row, relationIdx + 1).setValue(body.relation || '');

          logEvent(username, 'update_visitor', '', { name: body.name, idCard: idCard, oldIdCard: oldIdCard });
          return jsonResp({ status: 'ok', message: 'แก้ไขผู้เยี่ยมสำเร็จ' });
        }
      }
      return jsonResp({ status: 'error', message: 'ไม่พบผู้เยี่ยมที่ต้องการแก้ไข' });
    } catch(e) {
      return jsonResp({ status: 'error', message: 'แก้ไขผู้เยี่ยมไม่สำเจ็จ: ' + e.toString() });
    }
  }
